...
async function main() {
  ...
  let DEFI = await ethers.getContractFactory("PropertyCompoundV1", deployer);
  ...
  await usdt.mint(deployer.address, 1500);

  console.log("USDT minted :)");

  await defi.setAvailableInvestment(usdt.address, 50, 30);
